    ...
    pub async fn handle_command(&self, command: ServiceCommand<P>) -> ServiceEvent<P> {
        match command {
            ...
            ServiceCommand::Snapshot { vm_id, name } => match self.pool.get(&vm_id).await {
                ...
                // The one site with no error to ask: the pool answered
                // "no such VM" by returning nothing.
...
mod tests {
    ...
    async fn handle_allocate_exhausted() {
        ...
        match resp {
            // The kind is the field a caller decides on; the message is prose
            // beside it. A full pool is `Capacity` and nothing else — that is
            // what lets a client tell "no room right now" from "no such
            // image", which as English differ only as English.
