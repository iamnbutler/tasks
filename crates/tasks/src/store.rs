...
impl Store {
    ...
    pub async fn open(path: impl AsRef<Path>) -> Result<Self, StoreError> {
        ...
        let pool = SqlitePoolOptions::new()
            ...
            .await?;
        // `migrate_reporting` runs the migrations and diffs what *this* boot
        // applied, so it must be the only caller of `MIGRATOR.run`: a second
        // run ahead of it leaves the diff empty and the report silently blank.
        // Maintenance follows, because the markers it consumes are inserted by
        // the migrations it just reported.
