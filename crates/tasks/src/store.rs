...
///
/// Keyed on `rowid`, not `(session_id, seq)`: since #825 a transcript line is
/// owned by *either* a session or a build, so a build-owned row has
/// `session_id IS NULL` and every comparison against it is NULL — a
/// session-keyed sweep skips the entire build half of the table while
/// reporting success. `rowid` is owner-agnostic and covers both.
///
...
/// path. Paging is keyset rather than `OFFSET`: a rewritten row still matches
/// the prefilter, but nothing here should depend on that.
async fn sweep_transcript_credentials(pool: &SqlitePool) -> Result<u64, StoreError> {
    let mut cursor = i64::MIN;
    let mut scrubbed = 0u64;
    loop {
        let rows: Vec<(i64, String)> = sqlx::query_as(
            "SELECT rowid, line FROM transcript_lines \
             WHERE rowid > ? AND line LIKE '%://%@%' \
        ...
        for (rowid, line) in rows {
            ...
            if redacted != line {
                sqlx::query("UPDATE transcript_lines SET line = ? WHERE rowid = ?")
                    .bind(&redacted)
                    .bind(rowid)
                    .execute(&mut *tx)
                    .await?;
...
impl Store {
    ...
    pub async fn insert_project(&self, project: &Project) -> Result<(), StoreError> {
        sqlx::query(
            "INSERT INTO projects (id, repo_owner, repo_name, status, added_at) \
             VALUES (?, ?, ?, ?, ?)",
        )
    ...
    pub async fn get_project(&self, id: &ProjectId) -> Result<Option<Project>, StoreError> {
        let row = sqlx::query(
            "SELECT id, repo_owner, repo_name, status, added_at FROM projects WHERE id = ?",
        )
        .bind(id.as_str())
    ...
    /// than a duplicate row: [`crate::server`]'s `resolve_project` stops being
    /// able to answer "the only one there is", the poller ingests every issue
    /// twice, and every list shows the repo twice. Both add paths (the handler
    /// and `tasks add-project`) ask this first.
    pub async fn find_project_by_repo(
        ...
        let row = sqlx::query(
            "SELECT id, repo_owner, repo_name, status, added_at FROM projects \
    ...
    pub async fn list_projects(&self) -> Result<Vec<Project>, StoreError> {
        let rows = sqlx::query(
            "SELECT id, repo_owner, repo_name, status, added_at FROM projects ORDER BY added_at",
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    /// Pause, archive, or reactivate a repo. Returns the project as it stands
    /// afterwards; `NotFound` if there is no such project.
    ///
    /// Gates **new** work only, like the global mode: a scout or a build
    /// already in flight for this repo runs to its own conclusion.
    pub async fn set_project_status(
        &self,
        id: &ProjectId,
        status: ProjectStatus,
    ) -> Result<Project, StoreError> {
        let updated = sqlx::query("UPDATE projects SET status = ? WHERE id = ?")
            .bind(status.as_str())
            .bind(id.as_str())
            .execute(&self.pool)
            .await?;
    ...
    /// Write a spec by hand and approve it in the same act, for a task whose
    /// issue body already is the specification (#869).
    ///
    /// One transaction, because from the human's side this is one decision:
    /// the spec lands with `session_id NULL` (no Scout ran), its queue entry
    /// lands `approved`, the ledger gets a single `author_spec` row, and the
    /// task moves to `ready_to_build` for an ordinary `create_build` to pick
    /// up. What is skipped is not only the scouting but the **review** — there
    /// is no independent artifact to rule on, so the human writing this is the
    /// review, and the ledger says `author_spec` rather than `approve`
    /// precisely so nobody later reads a second opinion into it.
    ///
    /// `files_touched` stays empty and is not a placeholder to fill in later:
    /// [`crate::brief`] derives its overlap facts from that list, and an
    /// invented one would feed the brief a lie rather than an omission.
    ///
    /// Legal only from `Backlog` or `Queued` — the two states from which no
    /// Scout has run and none is running. A `Queued` task may already carry a
    /// spec (a `needs_revision` verdict returns it there), and that is allowed:
    /// the human read the feedback and decided to write it themselves. The
    /// older spec keeps its verdict; `latest_spec` and `create_build` see only
    /// this one. Every refusal happens before the transaction opens.
    pub async fn author_spec(
        ...
        if content.trim().is_empty() {
            return Err(StoreError::Invalid(
                "a hand-written spec needs content — there is nothing for the Builder to \
                 read otherwise"
                    .into(),
            ));
        }
        ...
        if !matches!(task.state, TaskState::Backlog | TaskState::Queued) {
            return Err(StoreError::Invalid(format!(
                "task {task_id} is {}, and build-now only writes a spec for work no Scout \
                 has started — review the spec it already has instead",
                task.state.as_str()
            )));
        }
        ...
        .execute(&mut *tx)
        .await?;
        // The ledger row lands with the writes it authorizes, as in
        // `review_spec`: this is the only judgment anywhere in this path, so a
        // crash that kept the approved spec and lost the reason would leave
        // nothing to review it by.
        let decision_seq = insert_decision(
            &mut tx,
            "spec",
            spec.id.as_str(),
        ...
        .await?;
        if task.state != TaskState::ReadyToBuild {
            self.append_event(EventPayload::TaskStateChanged {
                task_id: task.id.clone(),
                from: task.state,
                to: TaskState::ReadyToBuild,
    ...
    ///
    /// A build whose project does not [`ProjectStatus::dispatches`] is skipped
    /// **inside this transaction**, not claimed and released: claim-then-release
    /// would flip the build `queued → running → queued` on every tick of the
    /// build loop, and each claim drags its batch's tasks to `building`, so a
    /// paused repo would look like it was building forever. The `ORDER BY` is
    /// unchanged, so a paused repo's build simply does not answer the query and
    /// the one behind it does — pausing one repo is not pausing the lane.
    pub async fn claim_next_queued_build(&self) -> Result<Option<Build>, StoreError> {
        ...
        let Some(row) = sqlx::query(
            "SELECT b.id FROM builds b JOIN projects p ON p.id = b.project_id \
             WHERE b.status = ? AND p.status = ? ORDER BY b.created_at, b.rowid LIMIT 1",
        )
        .bind(BuildStatus::Queued.as_str())
...
fn spec_from_row(row: sqlx::sqlite::SqliteRow) -> Result<Spec, StoreError> {
    ...
    Ok(Spec {
        ...
        // NULL is the tell that no Scout ran — a human wrote this one.
        session_id: row
...
mod tests {
    ...
    async fn transcript_seqs_run_per_owner_and_never_cross() {
        let store = Store::open_in_memory().await.unwrap();
        let (_task, spec) = seed_spec(&store, 1).await;
        let session_owner = TranscriptOwner::session(spec.session_id.as_ref().unwrap());
    ...
    async fn the_owner_arc_is_exclusive_and_each_side_cascades() {
        ...
        for (session_id, build_id) in [
            (None, None),
            (
                Some(spec.session_id.as_ref().unwrap().to_string()),
    ...
    async fn seed_credentialed_session(store: &Store, number: u64) -> SessionId {
    ...
    async fn transcript_lines_are_scrubbed_on_the_way_in() {
        let store = Store::open_in_memory().await.unwrap();
        let session_id = seed_credentialed_session(&store, 1).await;
        let mut rx = store.subscribe_transcript();
        ...
        let read_back = store
            .transcript_since(&TranscriptOwner::session(&session_id), 0, 10)
            .await
            .unwrap();
    ...
    /// The half of the table #840 could not have known about.
    ///
    /// #840 was cut from a tree where a transcript line was always owned by a
    /// session, so its sweep paged and updated on `(session_id, seq)`. #825
    /// then made ownership an arc: a build-owned row has `session_id IS NULL`,
    /// and every comparison against NULL is NULL, so a session-keyed sweep
    /// walks straight past the build half while reporting success — the worst
    /// shape for a security repair, because the log says it scrubbed and the
    /// tokens are still there. Keyed on `rowid` it covers both.
    #[tokio::test]
    async fn the_sweep_scrubs_build_transcripts_too() {
        let store = Store::open_in_memory().await.unwrap();
        let (_task, spec) = seed_spec(&store, 1).await;
        approve(&store, &spec).await;
        let build = store
        ...
        // Written the way a pre-fix build did: straight in, no scrub.
        sqlx::query(
            "INSERT INTO transcript_lines (build_id, seq, timestamp, stream, line) \
             VALUES (?, 1, ?, 'stderr', ?)",
        )
        .bind(build.id.as_str())
        .bind(Utc::now().to_rfc3339())
        ...
        let lines = store
            .transcript_since(&TranscriptOwner::build(&build.id), 0, 10)
            .await
            .unwrap();
        assert_eq!(
            lines[0].line, SCRUBBED,
            "a build-owned row must be scrubbed, not skipped"
        );
        let leaked: i64 =
    ...
    /// The write path, on the other owner. `TranscriptSink::push` is shared
    /// between scouts and builds since #825, so one scrub covers both — this
    /// pins that it actually does.
    #[tokio::test]
    async fn build_transcript_lines_are_scrubbed_on_the_way_in() {
        let store = Store::open_in_memory().await.unwrap();
        let (_task, spec) = seed_spec(&store, 1).await;
        approve(&store, &spec).await;
        let build = store
    ...
    async fn the_sweep_pages_past_its_batch_size() {
        ...
        let a = seed_credentialed_session(&store, 1).await;
        let b = seed_credentialed_session(&store, 2).await;
    ...
    /// A build whose project is not `active` is skipped **inside the claim**,
    /// not claimed and released. Claim-then-release would flip the build
    /// `queued → running → queued` on every tick of the build loop, and each
    /// claim drags its batch's tasks to `building` — a paused repo would look
    /// like it was building forever.
    #[tokio::test]
    async fn a_paused_project_is_skipped_inside_the_claim() {
        let store = Store::open_in_memory().await.unwrap();
        let project = sample_project();
        store.insert_project(&project).await.unwrap();
        ...
        // Nothing moved: not the build, and not the batch's tasks.
        assert_eq!(
        ...
        // Reactivating hands it straight back — the row was never disturbed.
        store
    ...
    /// A paused repo at the head of the build queue must not starve the ones
    /// behind it — the same rule `next_dispatchable` follows for scouts.
    #[tokio::test]
    async fn a_paused_project_at_the_head_does_not_block_the_repo_behind_it() {
        let store = Store::open_in_memory().await.unwrap();
        let paused = sample_project();
        store.insert_project(&paused).await.unwrap();
        let other = second_project(&store).await;

        let (_ta, first) = approved_spec(&store, &paused, 1).await;
        let (_tb, second) = approved_spec(&store, &other, 2).await;
        let blocked = store
            .create_build(
        ...
        let claimed = store.claim_next_queued_build().await.unwrap().unwrap();
        assert_eq!(
            claimed.id, behind.id,
            "the older build's repo is paused; the one behind it still runs"
        );
        assert_eq!(
            store.get_build(&blocked.id).await.unwrap().unwrap().status,
            BuildStatus::Queued
        );
    }

    /// Why mode stays global: the build lane is one lane for the whole server.
    /// A per-repo `play` could not run while another repo's build held it, so
    /// the setting would promise what the architecture cannot deliver — which
    /// is the argument for a per-repo *subtraction* and nothing more.
    #[tokio::test]
    async fn one_repos_running_build_holds_the_lane_against_every_other_repo() {
        ...
        let second = second_project(&store).await;

        let (_ta, spec_a) = approved_spec(&store, &first, 1).await;
        let (_tb, spec_b) = approved_spec(&store, &second, 2).await;
        store
            .create_build(
                std::slice::from_ref(&spec_a.id),
                "main",
        ...
        assert!(store.claim_next_queued_build().await.unwrap().is_some());
        assert!(
            store.claim_next_queued_build().await.unwrap().is_none(),
            "the second repo's build waits on the first repo's, whatever either \
             project's status says"
        );
    }

    /// `UNIQUE(repo_owner, repo_name)` is case-sensitive, so the lookup that
    /// guards both add paths must not be.
    #[tokio::test]
    async fn a_project_is_found_whatever_case_the_repo_is_named_in() {
        let store = Store::open_in_memory().await.unwrap();
        let project = sample_project();
        store.insert_project(&project).await.unwrap();
    ...
    /// The status round-trips through the column, and an unknown project is a
    /// `NotFound` rather than a silent no-op.
    #[tokio::test]
    async fn project_status_round_trips_and_refuses_an_unknown_project() {
        let store = Store::open_in_memory().await.unwrap();
        let project = sample_project();
        store.insert_project(&project).await.unwrap();
        assert_eq!(
        ...
        for status in ProjectStatus::ALL {
            store.set_project_status(&project.id, status).await.unwrap();
            assert_eq!(
                store.list_projects().await.unwrap()[0].status,
                status,
                "{status:?}"
            );
        }
    ...
    /// A spec a human wrote is buildable by exactly the ordinary path: nothing
    /// downstream of `author_spec` knows or cares that no Scout ran.
    #[tokio::test]
    async fn a_hand_written_spec_builds_like_any_other() {
        let store = Store::open_in_memory().await.unwrap();
        let project = sample_project();
        store.insert_project(&project).await.unwrap();
        let task = sample_task(&project.id);
        ...
        // Round-trips through SQL as NULL, not as an empty string.
        assert_eq!(
            store.get_spec(&spec.id).await.unwrap().unwrap().session_id,
            None
        );

        // `create_build` accepts it on its own terms — it only ever asked
        // whether the queue entry was approved.
        let build = store
            .create_build(
                std::slice::from_ref(&spec.id),
                "main",
        ...
        // The events a client needs to follow this, in order, and the
        // provenance rides the first one.
        let kinds: Vec<&str> = store
            ...
            .collect();
        assert!(kinds.contains(&"spec_created"));
        assert!(kinds.contains(&"spec_queue_status_changed"));
        assert!(kinds.contains(&"task_state_changed"));
        let created = store
            .all_events()
            .await
            .unwrap()
            ...
            .find_map(|e| match e.payload {
                EventPayload::SpecCreated { session_id, .. } => Some(session_id),
                _ => None,
            })
            .expect("a spec_created event");
        assert_eq!(created, None, "no session to name");
    }

    /// The two states a Scout has not touched are legal; everything else is
    /// refused. `Queued` is legal even when it already carries a spec — a
    /// `needs_revision` verdict returns a task there, and the human who read
    /// that feedback may well decide to write the spec themselves.
    #[tokio::test]
    async fn author_spec_accepts_only_the_states_before_a_scout() {
        let store = Store::open_in_memory().await.unwrap();
        let project = sample_project();
        store.insert_project(&project).await.unwrap();

        for (issue, state) in [(1, TaskState::Backlog), (2, TaskState::Queued)] {
            let mut task = sample_task(&project.id);
            task.gh_issue_number = issue;
            task.state = state;
            store.insert_task(&task).await.unwrap();
            store
                ...
                .await
                .unwrap_or_else(|e| panic!("{} should be legal: {e}", state.as_str()));
        }

        for (issue, state) in [
            (10, TaskState::Scouting),
            (11, TaskState::InReview),
            (12, TaskState::ReadyToBuild),
            (13, TaskState::Building),
            (14, TaskState::Done),
            (15, TaskState::Rejected),
        ] {
            let mut task = sample_task(&project.id);
            task.gh_issue_number = issue;
            task.state = state;
        ...
        // A `queued` task that already carries a scout's rejected spec still
        // qualifies: the human read the verdict and is writing their own.
        let (task, scouted) = approved_spec(&store, &project, 99).await;
        store
            .review_spec(
                &scouted.id,
                SpecQueueStatus::NeedsRevision,
        ...
        let mine = store
            .author_spec(
                &task.id,
                "## Spec\nWhat the scout should have written.",
                Complexity::Simple,
                DecisionInput::human(),
            )
            .await
        ...
        assert_eq!(
            store
                .get_spec_queue_entry(&scouted.id)
                .await
                .unwrap()
                .unwrap()
                .status,
            SpecQueueStatus::NeedsRevision,
            "the older spec keeps its verdict"
        );
        assert_eq!(
            store
                .latest_reviewed_spec(&task.id)
                ...
                .id,
            mine.id,
            "the newest spec is the one anything downstream sees"
        );
    }

    /// Every refusal happens before the transaction opens, so a rejected call
    /// leaves nothing at all: no spec, no queue entry, no ledger row, no
    /// event, and the task where it was.
    #[tokio::test]
    async fn a_refused_author_spec_writes_nothing() {
        let store = Store::open_in_memory().await.unwrap();
        let project = sample_project();
        store.insert_project(&project).await.unwrap();
        let task = sample_task(&project.id);
        ...
        // An orchestrator that somehow reached the store still owes a
        // rationale, on the same rule as every other decision.
        assert!(matches!(
            store
                .author_spec(
                    &task.id,
