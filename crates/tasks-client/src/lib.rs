...
impl Client {
    ...
    /// pipeline. Human-only at the server, like [`Client::create_project`].
    ///
    /// Not a second play/pause: [`Mode`] is global, and this only ever takes
    /// work *away* from the dispatcher for one repo.
    pub fn set_project_status(&self, id: &ProjectId, status: ProjectStatus) -> Result<Project> {
        self.post_json(
