...
| `GITHUB_OAUTH_URL` | `https://github.com` | where the device flow speaks to GitHub — `tasks auth login`, and the server's own sign-in surface (`POST /auth/github/device`, human-only, driven by the app's Server window; the server polls and seals so the token never transits the app). The OAuth endpoints live on github.com rather than the API host, so `GITHUB_API_URL` is deliberately not reused — override for tests only |
