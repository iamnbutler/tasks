...
- **`done` means shipped, not "a PR exists".** A successful build parks its
  ...
  time and resolves it. Merged → the server closes the issue as completed,
  and the next poll retires the task to `done` through the ordinary
  closure-derived path. Closed unmerged → the batch's specs go back to
  `approved` (with a build attempt charged) and the tasks to
  `ready_to_build`, which restores the *option* to rebuild; nothing
