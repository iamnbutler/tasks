...
| Read credential custody | `GET /secrets` | Names, `set_at`, the key-source line, and per name what is **currently** serving it (`sealed` \| `environment` \| `api_key_helper` \| `unset`) — a four-variant enum, because "sealed, but the environment serves it" is a real state and the one a human most needs to see. **Never a value**, and there is deliberately no read-one route: no type in the wire vocabulary can carry a value outbound, so write-only is structural rather than a rule somebody remembers. 200 even with no store (`initialized: false`); 503 without a custody service. **Human-only** (403 otherwise) — it carries no value, so this is not a leak being closed, it is the surface not being advertised: it names the key source and the store path. |
| Paste a credential | `POST /secrets/{name}` `{"value"}` | Seal it, replacing any previous entry; the running server picks the write up off the store's mtime, with no restart. **204** ordinarily, **201** `{store_path, key_source}` when this call created the store (auto-init goes through `secrets::init` itself, so a pasted store cannot pick a different key source than a terminal-created one), **200** `{name, detail}` when the write landed and this process cannot read it back until a restart — a 2xx and never a 5xx, because the ciphertext *is* on disk and "your paste did not work" would have the human paste it again forever. 400 unknown name or empty value, 503 with no unseal key (and the message says it will not clear on its own). Refused when there is no store here but this host already holds a credential-store item, which auto-init would overwrite in place and strand. **Human-only.** |
| Remove a credential | `DELETE /secrets/{name}` | **204** whether or not anything was there. **Human-only.** |
